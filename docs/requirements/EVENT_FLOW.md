...
4. **중복 일정 확인** (핵심 검증)
   - 사용자가 참여 중인 모든 일정 조회
   - 각 일정의 trackingStartTime ~ eventTime 범위 확인
   - 신규 일정의 trackingStartTime ~ eventTime과 겹치는지 검증
...
B모임 1번 일정 참여 시도:
  ❌ 일정 시간 13:50 (13:35~13:50) → 겹침 (13:45~13:50)
  ❌ 일정 시간 14:00 (13:45~14:00) → 완전 중복
  ❌ 일정 시간 13:40 (13:25~13:40) → 겹침 (13:25~13:45, 20분 기준)
  ✅ 일정 시간 14:21 (14:06~14:21) → 가능 (겹치지 않음)
```
...
2. **중복 참여 불가**: 시간이 중복되는 다른 일정에는 참여 불가
